...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        lctr = torch.zeros(1, device=self.device)  # NEW center-regression L1 loss
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # --- Center L1 loss (NEW) ---
                txy = tbox[i][:, :2]                           # target centers
                lctr += nn.L1Loss()(pxy, txy)                  # L1 on center coords
        ...
        lobj *= self.hyp["obj"]
        lcls *= self.hyp["cls"]
        lctr *= self.center_weight                           # scale center loss
        bs = tobj.shape[0]  # batch size

        # include center loss in total
        loss = (lbox + lobj + lcls + lctr) * bs
        return loss, torch.cat((lbox, lobj, lcls, lctr)).detach()
