...
def train(hyp, opt, device, callbacks):
    ...
    # Log train/val dataset sizes
    LOGGER.info(f"train: {len(dataset)} images, {sum(len(l) for l in dataset.labels)} annotations")
    ...
    # Log val dataset size
    LOGGER.info(f"val:   {len(val_loader.dataset)} images, {sum(len(l) for l in val_loader.dataset.labels)} annotations")
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(("\n" + "%11s" * 8) % ("Epoch", "GPU_mem", "box_loss", "obj_loss", "cls_loss", "ctr_loss", "Instances", "Size"))
